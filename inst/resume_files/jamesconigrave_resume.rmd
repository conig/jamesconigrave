...
comm <- data.table::fread(
  "https://raw.githubusercontent.com/conig/jamesconigrave/master/inst/db/commentaries.csv",
  encoding = "Latin-1"
)
...
if (grants) {
  cat(
    "Select Grants {data-icon=file-invoice-dollar}
--------------------------------------------------------------------------------\n\n"
  )
...
```{r echo = FALSE, results = "asis"}
students <- data.table::fread(
  "https://raw.githubusercontent.com/conig/jamesconigrave/master/inst/db/students.csv"
)
...
```{r results = "asis", echo = FALSE, message = FALSE}
if (peer_review) {
  reviews <- data.table::fread(
    "https://raw.githubusercontent.com/conig/jamesconigrave/master/inst/db/peer%20review.csv"
  )
  reviews <- reviews[reviews$IF >= 2 & !is.na(reviews$IF), ][1:5, ]
  reviews <- reviews[with(reviews, order(Year, IF, decreasing = TRUE)), ]
  cat(
    "Recent peer review {data-icon=user-friends}
-------------------------------------------------------------------------------\n\n"
  )
...
--------------------------------------------------------------------------------\n\n"
  ...
  conf <- data.table::fread(
    "https://raw.githubusercontent.com/conig/jamesconigrave/master/inst/db/conference.csv"
  ) |>
    dplyr::arrange(desc(year))
