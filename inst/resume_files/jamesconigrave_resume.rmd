...
```{r results = "asis", echo = FALSE, message = FALSE}
if(peer_review){
  reviews <- read.csv(system.file("db/peer review.csv", package = "jamesconigrave"))
  reviews <- reviews[reviews$IF >= 1 & !is.na(reviews$IF),]
  reviews <- reviews[with(reviews, order(Year, Journal, decreasing = TRUE)),]
  cat("Peer review {data-icon=user-friends}
-------------------------------------------------------------------------------\n\n")
...
--------------------------------------------------------------------------------\n\n")
grnt = readr::read_csv(system.file("db/grants.csv",package = "jamesconigrave")) %>% 
  dplyr::arrange(desc(year))
