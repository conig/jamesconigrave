...
year_range <- function(start, end) {
  years <- as.character(unique(na.omit(c(start, end))))
  years <- years[nzchar(trimws(years))]
...
students$student_year_range <- mapply(
  year_range,
  students$year_start,
  students$year_end
)

content_header <- section_heading("HDR supervision", "users")
