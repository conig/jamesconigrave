...
pubs$author = lapply(seq_along(pubs[,"author"]), function(r){
  if(!grepl("\\.\\.\\.",pubs$author[r])) return(as.character(pubs$author[r]))

  authors = capture.output(cat(scholar::get_complete_authors(id, as.character(pubs$pubid[r]))))[2]
  Sys.sleep(sample(seq(1,2,by = 0.001),1)) # try to make pauses non-robotic
  jamesconigrave:::format_authors(authors)
  })
