---
title: "James Conigrave's resume"
author: James Conigrave
date: "`r Sys.Date()`"
output:
  pagedown::html_resume:
    self_contained: true
    yml_toc: true
    css: ["style-rules.css", "resume"]
---

```{r data-load, message=FALSE, warning=FALSE, echo=FALSE, include = FALSE}
library(dplyr); library(ggplot2)

id = "m0d4TKcAAAAJ"

#get pubs from google scholar
pubs = scholar::get_publications(id, flush = TRUE) %>%
  filter(journal != "ACU Research Bank") %>%
  mutate(author_last = gsub("^\\w{1,} *","",author)) %>% 
  mutate_if(is.factor, as.character) %>% 
  arrange(desc(year), author_last)

pubs$doi = jamesconigrave::get_doi(pubs)
pubs$doi = ifelse(!is.na(pubs$doi), paste0("https://doi.org/", pubs$doi), "")
pubs = lapply(pubs$doi, jamesconigrave::altmetric) %>% do.call(rbind,.) %>% 
  cbind(pubs, .)


# if there are any authors containing "..." get the full authorlist and format

pubs$author = scholar::get_complete_authors(id, pubs$pubid)

pubs$journal = ifelse(nchar(pubs$journal) > 0, paste0("*",pubs$journal,"*"), pubs$journal)
pubs$number = ifelse(nchar(pubs$number) > 0, paste0(" *",pubs$number,"*"), pubs$number)

# pubs$cite_word = ifelse(pubs$cites == 1, "citation","citations")
# pubs$cite_word = ifelse(pubs$cites == 0, "",pubs$cite_word)
# start_stats = ifelse(pubs$cites == 0, "N/A", paste("---",pubs$cites))

extra_info = function(row){
    info <- pubs[row, c("cites","tweets","media")]
    info[is.na(info)] = 0
    names(info) = c("citation","tweet","mainstream media outlet")
    names(info)[info > 0] = paste0(names(info)[info > 1],"s")
    info = subset(info, select = info > 1)
    plain_text <- paste(paste(info, names(info)), collapse = ", ")
    
    ifelse(nchar(plain_text) >0, paste0("--- ", plain_text), "N/A")
  }

pubs$extra_info = sapply(seq_along(pubs$doi), extra_info)


pubs$author = pubs$author %>% 
  gsub("J Conigrave", "JH Conigrave",.) %>% 
  gsub("JH Conigrave","<u>JH Conigrave<\\/u>",.) %>% 
  gsub("KS Lee","KSK Lee",.)

pre_prints = pubs[pubs$journal == "*PsyArXiv*",]
pubs =  pubs[pubs$journal != "*PsyArXiv*",]

# pattern = "### {title}
# 
# {journal}, {number} --- {cites} citations
# 
# N/A
# 
# {year}
# 
# {author}
# 
# "

pattern = "### {author} ({year}). {title}. {journal}{number}. {doi}

{extra_info}

N/A

{year}


"

fix_grammar = function(x) gsub("(?<!\\.)\\.\\.(?!\\.)","\\.",x, perl = T) %>% gsub("\\?\\.","\\?",.)
# regex interpretation. Match two dots and replace with one, if there's not an extra dot before, or after the two (three dots is a row).

publications = glue::glue(pattern, .envir = as.environment(pubs)) %>% 
   fix_grammar
  
  
pre_prints = glue::glue(pattern, .envir = as.environment(pre_prints)) %>% 
   fix_grammar

comm = read.csv(system.file("db/commentaries.csv", package = "jamesconigrave"), stringsAsFactors = F)
comm$extra_info = "N/A"
comm$number = ""
comm$doi = ""
comm$journal =  ifelse(nchar(comm$journal) > 0, paste0("*",comm$journal,"*"), comm$journal)

commentaries_other = glue::glue(pattern, .envir = as.environment(comm)) %>% 
  fix_grammar()

total_citations = scholar::get_profile(id)$total_cites
 
 h_index = scholar::predict_h_index(id)[1,2]
# cit_plot = ggplot(citations, aes(year, cites)) +
#   geom_bar(stat= "identity") + theme_classic() +
#   labs(y = "Citations",
#        x = "") +
#   scale_y_continuous(labels = scales::number_format(accuracy = 1)) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1))

CRAN_downloads = cranlogs::cran_downloads(c("corx","Conigrave"), from = "2015-01-01", to = lubridate::today()) %>% 
  group_by(package) %>% 
  summarise(dls = sum(count))

```

Aside
================================================================================

<!-- ![James](james_smaller.png){width=100%} -->

Contact Info {#contact}
--------------------------------------------------------------------------------

- <i class="fa fa-envelope"></i> james@conigrave.com
- <i class="fa fa-github"></i> [github.com/conig](https://github.com/JConigrave)
- <i class="fa fa-globe-asia"></i> [conigrave.com](https://conigrave.com)
<!-- - <i class="fa fa-phone"></i> +61 9515 9767 -->

Skills {#skills}
--------------------------------------------------------------------------------

- Statistical analysis, data-manipulation, dashboards, writing, public speaking

- Programming: R, Julia, Python

- Document creation: LaTeX, Markdown, HTML, CSS

- Adobe suite: Indesign, Photoshop, After effects 

Academic metrics {#skills}
--------------------------------------------------------------------------------

- Publications: `r nrow(pubs)`

- Citations: `r total_citations`

- h-index: `r h_index`

Disclaimer {#disclaimer}
--------------------------------------------------------------------------------

This HTML document was created using 'pagedown' in R. Publications are generated from Google Scholar using the 'scholar' library. [Check out the code on GitHub](https://github.com/conig/resume/blob/master/jamesconigrave_resume.rmd).

This document updates daily, get the latest version: [conigrave.com/resume](https://conigrave.com/resume).

Last updated: `r Sys.time()` AEDT

Main
================================================================================

James Conigrave, PhD {#title}
--------------------------------------------------------------------------------

### **Researcher --- Motivation, substance use and public health**

James is a Research Associate from the Centre for Research Excellence: Indigenous Health and Alcohol, at the University of Sydney. His research focuses using self-determination theory to understand alcohol consumption and dependence.


Education {data-icon=graduation-cap data-concise=true}
--------------------------------------------------------------------------------

### **PhD, Psychology**

Institute for Positive Psychology and Education,<br/> Australian Catholic University

North Sydney, NSW

2018 - 2015

<!-- Thesis: The Benefits of Believing You Can Change: Implicit Malleability Theories Moderate the Relationship Between Low self-esteem and Negative Outcomes -->

### **Masters of Public Health**

The University of Sydney

Camperdown, NSW

2012 - 2011

### **BA Psychology & B Health**

Macquarie University

Macquarie, NSW

2010 - 2007

Research Experience {data-icon=laptop}
--------------------------------------------------------------------------------

### **Research Associate**

Centre of Research Excellence: Indigenous Health + Alcohol, <br/>The University of Sydney

Camperdown, NSW

Present - 2018

### **Research Associate, Honorary**

Justice Health and Forensic Mental Health Network

Malabar, NSW

Present - 2019

<!-- - Developed algorithms to extract health data from prisoner reports -->

### **Research Assistant**

Institute for Positive Psychology and Education, <br/> Australian Catholic University

North Sydney, NSW

2017 - 2015

Professional Experience {data-icon=suitcase}
--------------------------------------------------------------------------------

### **Health Promotion Officer**

Sydney North Shore and Beaches Medicare Local

North Sydney, NSW

2014

<!-- - Managed GPs in school program -->

### **Mental Health Coordinator**

SNSB Medicare Local

Frenchs Forest, NSW

2014 - 2013

<!-- - Managed the Access To Allied Psychological Services (ATAPS) program -->


Teaching Experience {data-icon=chalkboard-teacher}
--------------------------------------------------------------------------------

### **Introductory statistics workshop**

Centre of Research Excellence: Indigenous Health + Alcohol, <br/>The University of Sydney

Camperdown, Sydney

2018

### **Heart Rate Variability (HRV) workshop**

Australian Catholic University

Strathfield, NSW

2016

Supervision {data-icon=users}
--------------------------------------------------------------------------------

### **Teagan Weatherall** 

PhD Student - Auxillary Supervisor

N/A

Present - 2019 

--- Primary supervisor is A/Prof. Kylie Lee

Committees {data-icon=comments}
--------------------------------------------------------------------------------

### **Australasian Professional Society of Alcohol and Other Drugs (APSAD)**

Early-Mid Career Researchers Subcomittee

N/A

Present - 2019

--- Elected Chair in 2020


R packages {data-icon=keyboard}
--------------------------------------------------------------------------------

### <u>JH Conigrave</u> (2019). corx: Create and Format Correlation Matrices. *CRAN*. https://conigrave.com/corx

--- `r CRAN_downloads$dls[2] %>% format(big.mark = ",")` downloads

N/A

2019


### <u>JH Conigrave</u> (2019). msemtools: Routines, tables, and figures for metaSEM analyses. *GitHub*. https://github.com/JConigrave/msemtools

N/A

N/A

2019

### <u>JH Conigrave</u> (2019). papyr: Tools for writing academic papers with rmarkdown. *GitHub*. https://github.com/JConigrave/papyr

N/A

N/A

2019

### <u>JH Conigrave</u> (2017). Conigrave: Flexible Tools for Multiple Imputation. *CRAN*. https://CRAN.R-project.org/package=Conigrave

--- `r CRAN_downloads$dls[1] %>% format(big.mark = ",")` downloads

N/A

2017

Publications {data-icon=file}
--------------------------------------------------------------------------------

```{r results = "asis", echo = FALSE, message = FALSE }

publications

```

Commentaries on work {data-icon=quote-left}
--------------------------------------------------------------------------------

```{r results = "asis", echo = FALSE, message = FALSE }

commentaries_other

```


Pre-prints {data-icon=file}
--------------------------------------------------------------------------------

```{r results = "asis", echo = FALSE, message = FALSE }

pre_prints

```

Peer review {data-icon=user-friends}
-------------------------------------------------------------------------------

### Journal of Health Science and Medical Research

N/A

N/A

2020

### Applied Psychology: Health and Well-Being

N/A

N/A

2019

### Journal of Ethnicity in Substance Abuse

N/A

N/A

2019

### Drug and Alcohol Dependence

N/A

N/A

2019

Conference contributions {data-icon=globe-asia}
--------------------------------------------------------------------------------

### <u>JH Conigrave</u>, KS Lee, K Harrison, T Dobbins, N Hayman, S Wilson, R Ivers, PS Haber, B Hummerston, D Johnson, D Gray, KM Conigrave (2019). Supporting Aboriginal Community Controlled Health Services to reduce the harms from alcohol. *The Australasian Professional Society on Alcohol & other Drugs (APSAD)*

N/A

Hobart, Australia

2019



### KSK Lee, S Wilson, <u>JH Conigrave</u>, J Perry, S Callinan, N Hayman, R Room, T Chikritzhs, D Gray, T Slade, KM Conigrave (2019). Re-thinking how we ask Indigenous Australia to describe their drinking. *The Australasian Professional Society on Alcohol & other Drugs (APSAD)*

N/A

Hobart, Australia

2019




### S Rombouts, K Morley, <u>JH Conigrave</u>, E Louie, PS Haber (2019). Evidence based models of care for the treatment of alcohol use disorder in primary health care settings: A systematic review and meta-analysis. *The Australasian Professional Society on Alcohol & other Drugs (APSAD)*

N/A

Hobart, Australia

2019


### M Dzidowska, KSK Lee, C Wylie, J Bailie, N Hayman, <u>JH Conigrave</u>, N Percival, KM Conigrave (2019). Approaches for improving practice in detection and treatment of unhealthy alcohol use in primary health care: Where are the gaps? *The Australasian Professional Society on Alcohol & other Drugs (APSAD)*

N/A

Hobart, Australia

2019


### <u>JH Conigrave</u>, BK Sahdra, J Ciarrochi (2017). The benefits of believing you can change: Growth mindset weakens the links between self-esteem and negative outcomes. *Contextual Behavioral Science Conference*

N/A

Seville, Spain

2017


### <u>JH Conigrave</u>, BK Sahdra, J Ciarrochi (2017). The benefits of believing you can change: Incremental theories weaken the links between self-esteem and negative outcomes. *Self Conference*

N/A

Melbourne, Australia

2017


Grants {data-icon=file-invoice-dollar}
--------------------------------------------------------------------------------

### Better data on methamphetamines and other drugs among Indigenous Australians to inform policy and practice

National Health and Medical Research Council

N/A

2019

KSK Lee, S Wilson, K Conigrave, T Chikritzhs, N Hayman, A Dawson, R Ali, <u>JH Conigrave</u> (CIH), M Fitts

--- \$2,630,000

### Early intervention for unhealthy alcohol use among older Aboriginal and Torres Strait Islander peoples: using the Grog Survey App

National Drug and Alcohol Research Centre

N/A

2019

S Farnbach, KSK Lee, A Eades, C Foley, <u>JH Conigrave</u> (CIE), S Wilson, K Conigrave, S Shakeshaft

--- \$40,000


